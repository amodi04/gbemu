...
void instr_to_str(cpu_context *ctx, char *str) {
    instruction *instr = ctx->curr_instr;
    sprintf(str, "%s ", inst_name(instr->type));
    ...
    switch(instr->mode) {
        ...
        case AM_R_D16:
        case AM_R_A16:
            sprintf(str, "%s %s,$%04X", inst_name(instr->type), 
                rt_lookup[instr->reg_1], ctx->fetched_data);
            return;

        case AM_R:
            sprintf(str, "%s %s", inst_name(instr->type), 
                rt_lookup[instr->reg_1]);
            return;

        case AM_R_R: 
            sprintf(str, "%s %s,%s", inst_name(instr->type), 
                rt_lookup[instr->reg_1], rt_lookup[instr->reg_2]);
            return;
        ...
        case AM_R_D8:
        case AM_R_A8:
            sprintf(str, "%s %s,$%02X", inst_name(instr->type), 
                rt_lookup[instr->reg_1], ctx->fetched_data & 0xFF);
            return;
        ...
        case AM_A8_R:
            sprintf(str, "%s $%02X,%s", inst_name(instr->type), 
                bus_read(ctx->regs.pc - 1), rt_lookup[instr->reg_2]);

            return;

        case AM_HL_SPR:
            sprintf(str, "%s (%s),SP+%d", inst_name(instr->type), 
                rt_lookup[instr->reg_1], ctx->fetched_data & 0xFF);
            return;

        case AM_D8:
            sprintf(str, "%s $%02X", inst_name(instr->type), 
                ctx->fetched_data & 0xFF);
            return;

        case AM_D16:
            sprintf(str, "%s $%04X", inst_name(instr->type), 
                ctx->fetched_data);
            return;
        ...
        case AM_A16_R:
            sprintf(str, "%s ($%04X),%s", inst_name(instr->type), 
                ctx->fetched_data, rt_lookup[instr->reg_2]);
            return;
